...
(function () {
  ...
  function renderPage() {
    ...
    if (!grid) {
      console.warn('[Achievements] No se encontró #achGrid en la página. ¿Está pegada la sección data-page="logros" en index.html?');
      return;
    }
    if (!DATA().length) {
      grid.innerHTML = '<div class="err" style="display:block;">⚠️ No se han cargado los logros (ACHIEVEMENTS_DATA vacío). Comprueba que js/achievements.js esté incluido en index.html justo antes de &lt;/body&gt; y refresca con Ctrl/Cmd+Shift+R.</div>';
      return;
    }
    check();
