...
import { parseEuro, getCssVar, hexToRgba, normalizeColorId, mainTypeBucket, bestSellPriceText } from '../core/utils.js';
...
export function updateInvestmentUI() {
  let invAll = 0, invNoProxy = 0, bestSellAll = 0;
  for (const c of state.cards) {
    const qty = c.qty || 1;
    const eur = parseEuro(c.priceText);
    if (Number.isFinite(eur)) {
      invAll     += eur * qty;
      if (!c.proxy) invNoProxy += eur * qty;
    }
    const bs = parseEuro(c.bestSell);
    if (Number.isFinite(bs)) bestSellAll += bs * qty;
  }
  const profit = bestSellAll - invAll;
  const el = id => document.getElementById(id);
  if (el('moneyInvestAll'))     el('moneyInvestAll').textContent     = `${invAll.toFixed(2)} €`;
  if (el('moneyInvestNoProxy')) el('moneyInvestNoProxy').textContent = `${invNoProxy.toFixed(2)} €`;
  if (el('moneyBestSellAll'))   el('moneyBestSellAll').textContent   = `${bestSellAll.toFixed(2)} €`;
  if (el('moneyProfitAll'))     el('moneyProfitAll').textContent     = `${profit.toFixed(2)} €`;
  if (el('invReal'))            el('invReal').textContent            = `${invNoProxy.toFixed(2)} €`;
...
export function renderTypeStats() {
  ...
  if (ctxT) {
    ...
    chartTypes = new Chart(ctxT, {
      type: 'bar',
      data: { labels: typeLabels, datasets: [{ label:'Tipos', data: typeLabels.map(k => typeCounts.get(k)), backgroundColor: hexToRgba(accent,.22), borderColor: hexToRgba(accent2,.90), borderWidth:1 }] },
  ...
  if (ctxR) {
    ...
    chartRarity = new Chart(ctxR, {
      type: 'doughnut',
      data: { labels: rarLabels, datasets: [{ label:'Rarezas', data: rarLabels.map(k => rarCounts.get(k)), backgroundColor:['rgba(255,255,255,.95)','rgba(220,220,220,.95)','rgba(255,225,160,.95)','rgba(255,170,120,.95)','rgba(248,187,208,.95)'], borderColor:'rgba(216,27,96,.35)', borderWidth:1 }] },
  ...
  if (ctxS) {
    ...
    chartSets = new Chart(ctxS, {
      type: 'bar',
      data: { labels: setEntries.map(e => e[0]), datasets: [{ label:'Cantidad', data: setEntries.map(e => e[1]), backgroundColor: getCssVar('--pink') }] },
