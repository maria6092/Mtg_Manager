...
import { escapeHtml, gradientFromColorIds, rarityClass, manaToEmojis, typesEs, colorToEmojisFromCI, computeColorId, manaValueFromCard, updatePriceHistory, bestSellPriceText, toTitleCase, parseEuro, sortCards, downloadFile, parseCsv } from '../core/utils.js';
import { RARITY_MAP, PASTEL } from '../core/constants.js';
...
export function renderCardsTable() {
  ...
  if (!tbody) return;
  tbody.innerHTML = '';
  for (const c of sortCards(state.cards, state.sortState)) {
    ...
    tr.innerHTML = `
      ...
      <td>
        <select data-k="lang">
          ${['es','en','fr','de','it','pt','ja','ko','zhs','zht'].map(l =>
            `<option value="${l}" ${c.lang===l?'selected':''}>${l}</option>`
          ).join('')}
        </select>
      </td>
...
export function initCardsTableListeners() {
  ...
  tbody.addEventListener('change', e => {
    ...
    const item = state.cards.find(x => x.id === tr.dataset.id); if (!item) return;
  ...
  tbody.addEventListener('click', e => {
    if (e.target.closest('[data-star]')) {
      ...
      item.fav = !item.fav;
      saveCards(); cloudSaveAll();
      renderCardsTable(); renderSalesTable();
      return;
    }
...
export function initAddCardButton() {
  ...
  document.getElementById('btnAdd')?.addEventListener('click', async () => {
    ...
    try {
      ...
      const qtyEl = document.getElementById('inQty'); if (qtyEl) qtyEl.value = '1';
      ['inProxy','inFoil','inPromo','inPromoPre','inSale'].forEach(id => {
        const el = document.getElementById(id); if (el) el.checked = false;
      });
      renderCardsTable();
      import('./collection-ui.js').then(m => m.renderColeccion());
  ...
  document.getElementById('btnExport')?.addEventListener('click', async () => {
    const { downloadFile: dl } = await import('../core/utils.js');
    dl('cartas_mtg.json', JSON.stringify(state.cards, null, 2));
  });
        ...
        for (const r of dataRows) {
          ...
          try {
            ...
            state.cards.unshift({
              ...
              rarity: RARITY_MAP[card.rarity] || card.rarity || '',
              logoUrl, priceText: price,
              proxy: false, foil, promo: false, promoPre: false,
              lang, qty, bestSell: bestSellPriceText(price),
              fav: false, sale: false, priceHist: [],
            });
